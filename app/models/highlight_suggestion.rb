class HighlightSuggestion < ApplicationRecord
  ...
  # hunt looks on Twitch for past broadcasts whose timestamps imply that it contains the run's PB. If found, the
  # highlight suggestion is saved to the database. If not, it is not.
  def hunt!
    return if run.user.nil? || run.user.twitch.nil?

    pb = run.histories.find_by(
      realtime_duration_ms: run.duration_ms(Run::REAL),
      gametime_duration_ms: run.duration_ms(Run::GAME)
    )
    return if pb.nil?

    Twitch::Videos.recent(run.user.twitch.twitch_id).each do |video|
      ...
      next unless video['type'] == 'archive'
