...
document.addEventListener("turbolinks:load", function() {
  ...
  runs.push(
    fetch(`/api/v4/runs/${gon.run.id}?historic=1&segment_groups=1`, {
      headers: { accept: "application/json" },
    })
  );

  if (gon.compare_runs !== undefined) {
    gon.compare_runs.forEach((run) =>
      runs.push(
        fetch(`/api/v4/runs/${run.id}?historic=1&segment_groups=1`, {
          headers: { accept: "application/json" },
        })
      )
    );
  ...
  Promise.all(runs)
    ...
    .then(function(bodies) {
      ...
      const timing =
        new URLSearchParams(window.location.search).get("timing") ||
        runs[0].default_timing;
      ...
      if (runs[0].histories.length !== 0) {
          ...
          segments: run.segments.map((segment, i) => ({
            ...
            filteredHistories: segment.histories.filter((attempt) => {
              ...
              if (run.segments[i - 1] !== undefined) {
                const prevSegAttempt = run.segments[i - 1].histories.find(
                  (prevAttempt) => {
                    // Find the previous splits attempt for this same attempt id to make sure it wasn't skipped
                    return (
                      prevAttempt.attempt_number === attempt.attempt_number
                    );
                  }
                );
                if (
                  prevSegAttempt !== undefined &&
                  prevSegAttempt[duration] === 0
                ) {
        ...
        Array.from(document.getElementsByClassName("segment-spinner")).forEach(
    ...
    .catch(function(error) {
      for (const segChartAlert of document.getElementsByClassName(
        "segment-chart-alert"
      )) {
      ...
      document.getElementById(
        "chart-alert"
      ).textContent = `Error loading charts: ${error}`;
...
document.addEventListener("click", (event) => {
  ...
  // not in the DOM.
  const segCharts = Highcharts.charts.filter(
    (chart) => chart.renderTo.id === `segment-duration-chart-${segId}`
  );
  segCharts.forEach((chart) => chart.reflow());
});

document.addEventListener("click", (event) => {
  const resetChartToggler = event.target.querySelector(
    'input[name="reset-chart-buttons"]'
  );
