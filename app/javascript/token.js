...
const urlHashToObject = function(hash) {
  ...
  hash
    .substring(1)
    .split("&")
    .map((hk) => (params[hk.split("=")[0]] = hk.split("=")[1]));
  return params;
};
...
document.addEventListener("turbolinks:load", () => {
  ...
  if (
    localStorage.getItem(accessTokenExpiryKey) !== null &&
    (expiry - now) / (1000 * 60) >= 5
  ) {
    return;
  }
  ...
  iframe.onload = () => {
    ...
    if (accessToken === "undefined") {
      console.warn(
        "Can't get a Splits.io access token. Your SPLITSIO_CLIENT_ID/SPLITSIO_CLIENT_SECRET may be wrong."
      );
      return;
    }
