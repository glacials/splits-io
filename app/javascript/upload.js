...
const upload = async function (file, options) {
  ...
  for (const [k, v] of Object.entries(
    splitsioResponse.presigned_request.fields
  )) {
    formData.append(k, v);
  }
  ...
  return fetch(splitsioResponse.presigned_request.uri, {
    ...
    .catch(function (error) {
      ...
      document.getElementById(
        "droplabel"
      ).innerHTML = `Error: ${error.message}.<br />Try again, or email help@splits.io!<br />`;
...
const uploadAll = function (files) {
  ...
  Promise.all(
    files.map(
      (file) =>
        new Promise((resolve, reject) => {
          upload(file, { bulk: true })
            .then((runId) => {
              const newTotal =
                Number(
                  document.getElementById("successful-uploads").textContent
                ) + 1;
              document.getElementById("successful-uploads").textContent =
                newTotal;
              resolve(runId);
            ...
            .catch((error) => {
              const newTotal =
                Number(document.getElementById("failed-uploads").textContent) +
                1;
    ...
    waitForRunsToParse(runIds).then(
      () => Turbolinks.visit("/"),
      () => Turbolinks.visit("/")
    );
  });
